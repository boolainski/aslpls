...
#!letters - show the current letters .
#
#INSTALARE :
#Lang Romain
...
set scrabble(end_rounds) "2"

set monchan "#nytcrwlrz"
...
bind pub - !letters arata:letters
...
proc arata:letters {nick host hand chan arg} {
	global scrabble
	
if {![channel get $chan scrabble]} {
	putserv "NOTICE $nick :Le \00312 Scrabble\003 is not activated, see an IRCop."
	return 0
}
if {![info exists scrabble($chan:run)]  && [channel get $chan scrabble]} {
  putserv "PRIVMSG $chan :\00312Type !scrabble to start the game."
  return 0
}
if {[info exists scrabble(word:$chan)]} {
	putserv "PRIVMSG $chan :\00304The current letters are \00301:\003 \00300,12 $scrabble(word:$chan) \003"
}	
}
...
if {![channel get $chan scrabble]} {
	putserv "NOTICE $nick : The \00312 Scrabble\003 is not activated."
	return 0
}
...
if {[info exists found_nrg]} {
	putserv "PRIVMSG $chan :\00312$user\003 \00304a\003 \00304$points\003 \00304points and\003 \00304$rounds\003 \00304rounds won.\003"
} else {
	putserv "NOTICE $nick :\00304No information found\003"
	}
}	
...
if {$option == ""} {
	putserv "NOTICE $nick :\00304Use\003 \00312!top <general> / <round>\003"
	return 0
}

switch -exact -- $option {
general {
	putserv "PRIVMSG $chan :\0031Top of \00300,2 Scrabble \003 General :"
	afiseaza:topscrabble $chan "general"
}
runde {
	putserv "PRIVMSG $chan :\0031Top of \00300,2 Scrabble \003 Tours :"
	afiseaza:topscrabble $chan "runde"
	}
default {
	putserv "NOTICE $nick :\00304Use\003 \00312!top <general>\003"
	}
...
if {$counter < 11} { 
        lappend the_line \00301 $counter \00301: \003\0031[join $topscr($t_scr) ,] \00304$t_scr\ \00301Points\003
}
}
...
if {[info exists the_line]} {
	...
	putserv "PRIVMSG $chan :\0031No player registered in the top10\003" 
	}
}
...
resetare:top $chan $arguments
	...
	putserv "NOTICE $nick :\00304Resetting your top10..\003"	
	return 0
}
if {[string equal -nocase "on" $option]} {
	channel set $chan +scrabble
	putserv "PRIVMSG $chan :The \00304 Scrabble\003 is activated. To start playing type \00304!scrabble\003"
	return 0
}
...
if {[string equal -nocase "off" $option]} {
	channel set $chan -scrabble
	scrabble:stop $chan
	putserv "PRIVMSG $chan :The \00312 Scrabble\003 is disabled."
	return 0
	}
}
...
if {[string equal -nocase "version" $option]} {
	putserv "PRIVMSG $chan :Version \00300,2 #Buangons v100000\003 ."
	return 0
}

if {[string equal -nocase "stop" $option]} {
   if { [matchattr [nick2hand $nick] "o" $chan]} {
	putserv "PRIVMSG $chan :\00304The Scrabble\003 was stopped by $nick."
	scrabble:stop $chan
   } else {
     putserv "NOTICE $nick :You do not have permission to stop Scrabble!"
   }
	return 0
}
if {[info exists scrabble($chan:run)]} {
			putserv "NOTICE $nick :\00304Scrabble is already running.\003"
			return 0
}          
	putserv "PRIVMSG $chan :\00300,2 ¤ Scrabble ¤ \003"
        putserv "PRIVMSG $chan :\00304Find as many words as possible in the allotted time.. To know the current letters type\00302 !letters \00302."
...
if {$data == ""} {
	...
	putserv "PRIVMSG $chan :\00308,4 ATTENTION \003 : \0031 Words\00304 Minimum\0031 from\00304 4\0031 letters\00301 : \00300,2 $scrabble(word:$chan) \003\00309 - \0031You have \00304$timer_seconds seconds \0031to find a \00304Maximum \0031of words."
...
#	puthelp "PRIVMSG $chan :\00304[join $now_voice ", "]\003 \0031receive a \00304VOICE (+)\003 \0031because there are in \00304Top 3 Scrabble\003 ."
#} else {
#	puthelp "PRIVMSG $chan :\00304$now_voice\003 \0031receive a \00304VOICE (+)\003 \0031because he is in \00304Top 3 Scrabble\003."
			}
		}
	}
}
...
#	utimer 3 [list puthelp "PRIVMSG $chan :\00304$nick\003 \0031receive a \00304VOICE (+)\003 \0031because he is in \00304Top 3 Scrabble\003 ."]
		}
	}	
}
...
putserv "PRIVMSG $chan :\0031The alternative answer was\00301 : \00304$scrabble(current_word:$chan)\003 \00301- \0031 Prepare your next word in a few seconds"
putserv "PRIVMSG $chan :\00304Round winners\0031 : \00304[join $nicks ", "]\003 \0031with \00304$max \00301Points\003"
	       runda:castigata $chan $winner($max)		
		}
		set scrabble(is:played:$chan) 1
	}
        putserv "PRIVMSG $chan :\0031Top 10\00304 Scrabble\00301 Players:\003"  
               afiseaza:topscrabble $chan "general"   
        putserv "PRIVMSG $chan :\00300,2 ¤ Scrabble ¤ \003 \00304 To know the current letters type\00302 !letters."
}
...
if {[info exists scrabble(no:playing:$chan)]} {
	...
	putserv "PRIVMSG $chan :\00312Scrabble\003 stopped. To play type \00304!scrabble\003 ."
	unset scrabble(no:playing:$chan)
if {[info exists scrabble(is:played:$chan)]} {
...
# modif pour couleur première letters de couleur différente de la suite
...
if {[lsearch -exact [string tolower $scrabble($chan:round_words)] [string tolower $cuvant_dat]] > -1} {
	putserv "PRIVMSG $chan :\0031Sorry \00304$orinick \0031word\003 \00304$cuvant_dat\003 \0031has already been said..\003"
	return 0
	}
}
...
if {[info exists exists_user] && [info exists scrabble($chan:the_time)]} {
	putserv "PRIVMSG $chan :\00304$nick\003 \0031with the word \00300,2 $cuvant_dat \003 \0031you obtain \00304$punctaj points\003 \0031on \00304[expr $scrabble($chan:the_time) - [get:scrabble:time $chan]] seconds.\003 \0031This tour \00301: \00304$scrabble($mask:current_points) points\003 \00301- \0031Tours won \00301: \00304$runde \00301- \0031Total points \00301: \00304$punctaj_citit . \0031Hurry, you have left \00304[get:scrabble:time $chan] seconds\003"
	}
}
